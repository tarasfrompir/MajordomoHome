...
class blockly_code extends module
{
    ...
    function admin(&$out)
    {
        ...
        if (isset($rec['CODE_TYPE'])) $out['CODE_TYPE'] = (int)$rec['CODE_TYPE'];
        if (!$rec && $this->owner->xml) {
            $rec['XML'] = $this->owner->xml;
        }
        if (isset($rec['XML']) and preg_match_all('/<block type="(.+?)\_turnoff"(.+?)>/uis',$rec['XML'],$m)) {
            $total = count($m[0]);
            for($i=0;$i<$total;$i++) {
                $new_line = $m[0][$i];
                $closed_block = 0;
        ...
        if (isset($rec['XML']) and preg_match_all('/<block type="(.+?)\_turnOn"(.+?)>/uis',$rec['XML'],$m)) {
            $total = count($m[0]);
            for($i=0;$i<$total;$i++) {
                $new_line = $m[0][$i];
                $closed_block = 0;
        ...
        if (isset($rec['XML'])) $rec['XML'] = preg_replace('/id="\?/', 'id="Q', $rec['XML']);
        $out['XML'] = $rec['XML'] ?? '';
        //$out['XML']='';
